...
@Component({
  selector: 'app-incoming-approval-form-page',
  templateUrl: './incoming-approval-form-page.component.html',
  styleUrls: ['./incoming-approval-form-page.component.css']
})
export class IncomingApprovalFormPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // adding incoming approval data
    this.addIncomingApprovalByID(this.route.snapshot.params['id']);
    this.addIncomingApprovalForm = this.formBuilder.group({
      // add incoming approvel forms
      incomingapprovelby: new FormControl('', Validators.required),
      shiftdate03: new FormControl('', Validators.required),
      incomingwithcomment: new FormControl('', Validators.required),
  ...
  addIncomingApprovalByID(id: any){
    this.api.getOneShiftHandoverbyID(id)
    .subscribe((data: any)=>{
      this.id = data.id;
      this.shifthandoverList = data;

      this.addIncomingApprovalForm.patchValue({
        // add incoming approvel forms
  ...
  get safetyLogs(){
    return this.addIncomingApprovalForm.get('safety_logs') as FormArray;
  }

  get overrideLogs(){
    return this.addIncomingApprovalForm.get('override_logs') as FormArray;
  }

  get permitToWorks(){
    return this.addIncomingApprovalForm.get('permit_to_works') as FormArray;
  }

  get standingInstructions(){
    return this.addIncomingApprovalForm.get('standing_instruction') as FormArray;
  }

  get operationCareChecklists(){
    return this.addIncomingApprovalForm.get('operation_care_checklist') as FormArray;
  }

  get highlightsOperationalLogs(){
    return this.addIncomingApprovalForm.get('highlight_operational_logs') as FormArray;
  }

  get maintenanceItems(){
    return this.addIncomingApprovalForm.get('maintenance_items') as FormArray;
  }
